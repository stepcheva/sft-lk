<template>
    <div class="flex receivers">
        <div class="receivers__col receivers__col-left">
            ...
            <div class="info-line-small">Диаметр рулона: <span class="info-line-small__color">{{ selected.roll_diameter}}</span>
            </div>
            <div class="info-line-small">Диаметр гильзы: <span class="info-line-small__color">{{ selected.core_diameter}}</span>
            </div>
        </div>
            ...
            <div class="infoblocks" v-for="product in products">
                <div class="infoblock flex">
                    <div class="infoblock__col">
                        ...
                        <div class="info-line-small">Марка: <span class="info-line-small__color">{{ product.brand
                            }}</span></div>
                        <div class="info-line-small">Граммаж: <span class="info-line-small__color">{{ product.grammage
                            }}</span></div>
                        <div class="info-line-small">Формат: <span class="info-line-small__color">{{ product.format
                            }}</span></div>
                    </div>
                    <div class="infoblock__col">
                        <div class="info-title infoblock__info-title">Объём продукции</div>
                        <div class="info-line-small">Минимальная партия: <span
                                class="info-line-small__color">{{ product.min_lot }} т</span></div>
                    </div>
                    <div class="infoblock__col">
                        <div class="info-title infoblock__info-title">Цена</div>
                        <div class="info-line-small" v-for="(delivery) in product.deliveries">{{ delivery.name }}:
                            <span class="info-line-small__color"> {{ delivery.price }}
...
<script>
    export default {
        ...
        methods: {
            getConsigneers() {
                let route = 'http://localhost/sft-lk/public/user/' + this.applicator + '/consigneers';
            ...
            getProduct() {
                let route = 'http://localhost/sft-lk/public/user/' + this.applicator + '/get/productranges/' + this.selected.id;
                axios.get(route).then((response) => {
